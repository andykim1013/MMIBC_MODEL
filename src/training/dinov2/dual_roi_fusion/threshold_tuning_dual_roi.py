...
def setup_logging(output_dir):
    ...
    log_dir = os.path.join(output_dir, f"logs_dual_roi_threshold_{current_time}")
...
def collect_scores(model, loader, device):
    ...
    with torch.no_grad():
        for mammo_batch, us_batch, us_roi_batch, labels in loader:
            ...
            logits = model(mammo_batch, us_batch, us_roi_batch)
...
def sweep_thresholds(labels, scores, thresholds):
    ...
    for threshold in thresholds:
        metrics, _ = metrics_from_threshold(labels, scores, threshold)
        rows.append(metrics)
...
def pick_thresholds(val_df, macro_tolerance=0.03, acc_tolerance=0.04):
    """
    Select validation-based thresholds.

    1. baseline_0.50:
       Normal argmax-equivalent threshold.

    2. best_macro_f1:
       Highest validation macro F1.

    3. best_malignant_f1:
       Highest validation malignant F1.

    4. recall_constrained:
       Highest validation malignant recall while keeping
       validation macro F1 and accuracy within allowed tolerance.

    5. recall_priority:
       Highest validation malignant recall while keeping
       validation accuracy >= 0.80 and macro F1 >= 0.75.
       This is more recall-oriented.
    """
    ...
    recall_priority_df = val_df[
        (val_df["accuracy"] >= 0.80)
        & (val_df["macro_f1"] >= 0.75)
    ].copy()

    if len(recall_priority_df) == 0:
        recall_priority = recall_constrained
    else:
        recall_priority = (
            recall_priority_df.sort_values(
                ["malignant_recall", "macro_f1", "accuracy"],
                ascending=[False, False, False],
            )
            .iloc[0]
...
def main(args):
    ...
    logging.info("Starting Dual-ROI threshold tuning")
    ...
    y_test, test_scores, _ = collect_scores(model, test_loader, device)
    ...
    roc_path = os.path.join(args.output_dir, "dual_roi_threshold_roc.png")
    plot_roc(y_test, test_scores, roc_path, "Dual-ROI threshold tuning ROC")

    summary = {
        "method": "4-2 Dual-ROI threshold tuning",
        "seed": int(args.seed),
    ...
    print("\n" + "=" * 80)
    print("4-2 DUAL-ROI THRESHOLD TUNING COMPLETE")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="4-2 Threshold tuning for Dual-ROI MMIBC model.")
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default=r"C:\Users\konyang\Desktop\MMIBC-main\saved_models\best_multimodal_model_dual_roi_ce_bs4_seed42_init_mmibc.pth",
    )

    parser.add_argument(
        "--output_dir",
        type=str,
        default=r"C:\Users\konyang\Desktop\MMIBC-main\runs\dual_roi_fusion\dual_roi_threshold_seed42_init_mmibc",
    )
