...
def setup_logging(output_dir):
    ...
    log_dir = os.path.join(output_dir, f"logs_dual_roi_{current_time}")
...
def set_train_mode(model, train_mode):
    ...
    train_mode:
        context_only:
            train only original-US + ROI-US context fusion block

        fusion_layers:
            train US context fusion + cross-attention + gated fusion + classifier

        classifier_only:
            train classifier only

        all:
            train all parameters, including DINOv2 encoders
            This is heavy and not recommended first.
    """
    ...
    if train_mode == "context_only":
        modules = [
            model.us_context_fusion,
        ]

    elif train_mode == "classifier_only":
        modules = [
...
def main(args):
    ...
    logging.info("Starting 4-1 Dual-ROI multimodal training")
    ...
    if args.init_model_path is not None and args.init_model_path != "" and os.path.exists(args.init_model_path):
        logging.info(f"Loading partial checkpoint from existing multimodal model: {args.init_model_path}")
        ...
        logging.info("No multimodal checkpoint loaded. Using pretrained unimodal encoders + fresh dual fusion layers.")
    ...
    for epoch in range(int(args.epochs)):
        ...
        for mammo_batch, us_batch, us_roi_batch, labels in tqdm(
            train_loader,
            desc=f"Epoch {epoch + 1} [Dual-ROI Train]",
        ):
            mammo_batch = mammo_batch.to(device)
            us_batch = us_batch.to(device)
            us_roi_batch = us_roi_batch.to(device)
    ...
    history_path = os.path.join(args.output_dir, "dual_roi_training_history.csv")
    history_df.to_csv(history_path, index=False, encoding="utf-8-sig")

    logging.info("Loading best Dual-ROI model for final clean test evaluation.")
    ...
    report_txt = (
        "=" * 80 + "\n"
        "4-1 Dual-ROI MMIBC Clean Test Result\n"
        "=" * 80 + "\n\n"
        f"Seed: {args.seed}\n"
    ...
    report_path = os.path.join(args.output_dir, "dual_roi_clean_test_report.txt")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report_txt)

    summary = {
        "method": "4-1 Dual-ROI MMIBC",
        "seed": int(args.seed),
        "config": args.config,
    ...
    summary_path = os.path.join(args.output_dir, "dual_roi_clean_summary.json")
    ...
    pred_path = os.path.join(args.output_dir, "dual_roi_clean_test_predictions.csv")
    pred_df.to_csv(pred_path, index=False, encoding="utf-8-sig")

    plot_confusion_matrix(
        np.array(test_metrics["confusion_matrix"]),
        os.path.join(args.output_dir, "cm_dual_roi_clean.png"),
        "4-1 Dual-ROI MMIBC Clean Test",
    )

    plot_roc(
        y_test,
        test_scores,
        os.path.join(args.output_dir, "roc_dual_roi_clean.png"),
        "4-1 Dual-ROI MMIBC ROC - Clean Test",
    )

    logging.info("Dual-ROI training finished.")
    ...
    # print("\n" + "=" * 80)
    # print("4-1 DUAL-ROI MMIBC CLEAN TEST RESULT")
    # print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="4-1 Dual-ROI MMIBC training.")
    ...
    parser.add_argument(
        "--roi_csv",
        type=str,
        default=r"C:\Users\konyang\Desktop\MMIBC-main\mmibc\multimodal_pairs_roi_margin030_no_mammo_leak.csv",
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=r"C:\Users\konyang\Desktop\MMIBC-main\runs\dual_roi_fusion\dual_roi_ce_bs4_seed42",
    )
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="best_multimodal_model_dual_roi_ce_bs4_seed42.pth",
    )
