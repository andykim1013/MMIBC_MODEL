...
#     mm_dir = os.path.join(root, "src", "training", "dinov2", "multimodal_model")
#     if mm_dir not in sys.path:
#         sys.path.append(mm_dir)

#     recall_dir = os.path.join(root, "src", "training", "dinov2", "recall_optimization")
#     if recall_dir not in sys.path:
...
#     log_dir = os.path.join(output_dir, f"logs_focal_{current_time}")
...
#     from src.training.dinov2.recall_optimization.losses import FocalLoss
...
#     logging.info("Starting 2-2 Focal Loss training")
#     logging.info(f"Using device: {device}")
#     logging.info(f"Config: {args.config}")
#     logging.info(f"CSV: {config['data']['csv_path']}")
#     logging.info(f"Gamma: {args.gamma}")
#     logging.info(f"Alpha malignant: {args.alpha_malignant}")
...
#     # 2-2 순수 Focal Loss 기본값: alpha 없음.
#     # alpha_malignant를 지정하면 alpha-balanced focal로 동작함.
#     if args.alpha_malignant is None:
#         alpha = None
#         logging.info("Using pure Focal Loss without alpha balancing.")
#     else:
#         alpha_m = float(args.alpha_malignant)
#         alpha = [1.0 - alpha_m, alpha_m]
#         logging.info(f"Using alpha-balanced Focal Loss: alpha=[benign={alpha[0]}, malignant={alpha[1]}]")
...
#     # 기존 MMIBC 흐름 유지: fusion classifier만 학습
#     optimizer = optim.AdamW(
#         model.fusion_classifier.parameters(),
#         lr=float(args.learning_rate) if args.learning_rate is not None else float(config["training"]["learning_rate"]),
#     )
...
#     max_epochs = int(args.epochs) if args.epochs is not None else int(config["training"]["epochs"])
#     patience = int(args.patience) if args.patience is not None else int(config["training"]["early_stopping"]["patience"])
...
#         for mammo_batch, us_batch, labels in tqdm(train_loader, desc=f"Epoch {epoch + 1} [Focal Train]"):
#             mammo_batch = mammo_batch.to(device)
#             us_batch = us_batch.to(device)
#             labels = labels.to(device)
...
#     history_path = os.path.join(args.output_dir, "focal_training_history.csv")
#     history_df.to_csv(history_path, index=False, encoding="utf-8-sig")

#     logging.info("Loading best focal model for final clean test evaluation.")
...
#     report_txt = (
#         "=" * 80 + "\n"
#         "2-2 Focal Loss Clean Training Result\n"
#         "=" * 80 + "\n\n"
#         f"Best epoch: {best_epoch}\n"
#         f"Gamma: {args.gamma}\n"
#         f"Alpha malignant: {args.alpha_malignant}\n"
...
#     report_path = os.path.join(args.output_dir, "focal_clean_test_report.txt")
...
#         "gamma": float(args.gamma),
#         "alpha_malignant": None if args.alpha_malignant is None else float(args.alpha_malignant),
...
#     summary_path = os.path.join(args.output_dir, "focal_clean_summary.json")
...
#     pred_path = os.path.join(args.output_dir, "focal_clean_test_predictions.csv")
#     pred_df.to_csv(pred_path, index=False, encoding="utf-8-sig")

#     plot_confusion_matrix(
#         np.array(test_metrics["confusion_matrix"]),
#         os.path.join(args.output_dir, "cm_focal_clean.png"),
#         "2-2 Focal Loss Clean Test",
#     )

#     plot_roc(
#         y_test,
#         test_scores,
#         os.path.join(args.output_dir, "roc_focal_clean.png"),
#         "2-2 Focal Loss ROC - Clean Test",
#     )

#     logging.info("Focal training finished.")
...
#     print("\n" + "=" * 80)
#     print("2-2 FOCAL LOSS CLEAN TEST RESULT")
#     print("=" * 80)
...
#     parser = argparse.ArgumentParser(description="2-2 Focal Loss training for MMIBC multimodal model.")

#     parser.add_argument(
#         "--root",
#         type=str,
#         default=r"C:\Users\konyang\Desktop\MMIBC-main",
#     )

#     parser.add_argument(
#         "--config",
#         type=str,
#         default=r"C:\Users\konyang\Desktop\MMIBC-main\src\training\dinov2\multimodal_model\config_no_mammo_leak.yaml",
#     )

#     parser.add_argument(
#         "--output_dir",
#         type=str,
#         default=r"C:\Users\konyang\Desktop\MMIBC-main\runs\recall_optimization\focal_loss_clean",
#     )

#     parser.add_argument(
#         "--model_save_dir",
#         type=str,
#         default=r"C:\Users\konyang\Desktop\MMIBC-main\saved_models",
#     )

#     parser.add_argument(
#         "--model_name",
#         type=str,
#         default="best_multimodal_model_focal_clean.pth",
#     )
...
#         type=float,
#         default=None,
#         help="Optional alpha for malignant class. If omitted, pure focal loss is used.",
#     )
...
def main(args):
    ...
    logging.info(f"Learning rate: {args.learning_rate if args.learning_rate is not None else config['training']['learning_rate']}")
    logging.info(f"Model save path: {model_save_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=r"C:\Users\konyang\Desktop\MMIBC-main\runs\recall_optimization\focal_loss_clean_seed42",
    )
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="best_multimodal_model_focal_clean_seed42.pth",
    )
